...
\begin{description}
  \item[\refstepcounter{acnum} \actheacnum \label{acOutput}:] Support for additional MuJoCo environments.
  \item[\refstepcounter{acnum} \actheacnum \label{acOutput}:] Simulation support for all types of operating systems.
  \item[\refstepcounter{acnum} \actheacnum \label{acOutput}:] Comprehensive logging across all of TPG's components.
  \item[\refstepcounter{acnum} \actheacnum \label{acVerify}:] Extensive testing and checks for newly published code.
  \item[\refstepcounter{acnum} \actheacnum \label{acVerify}:] Refactoring old and unstructured components.
...
\begin{description}
  \item[\refstepcounter{ucnum} \uctheucnum \label{ucControl}:] Migration of TPG's implementation to a different language.
  \item[\refstepcounter{ucnum} \uctheucnum \label{ucControl}:] Refactoring the flow of task initialization and generation.
  \item[\refstepcounter{ucnum} \uctheucnum \label{ucVerify}:] Transitioning away from Github Actions to other CI/CD platforms.
  \item[\refstepcounter{ucnum} \uctheucnum \label{ucControl}:] The replacement of MPI as the distributed computing framework.
  \item[\refstepcounter{ucnum} \uctheucnum \label{ucControl}:] Changing of communication protocol between different components such as gRPC.
\end{description}
